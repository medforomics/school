...
print OUT join("\t",'Sample','total.raw','total.trimmed','pairs','maprate','propair','Percent.ontarget','Mean.MapQ',
	       'Percent.QualReads','Median.Mismatch','Indel.Rate','Error.Rate','Percent.Dups','Library.Size',
	       'Subset.Percent.Dups','Median.Insert','Mean.Insert','Std.insert',
	       'all.avg.depth','all.median.depth','all.perc.10x','all.perc.50x','all.perc.100x',
	       'all.perc.200x','all.perc.500x','dedup.avg.depth','dedup.median.depth',
	       'dedup.perc.10x','dedup.perc.50x','dedup.perc.100x','dedup.perc.200x','dedup.perc.500x'),"\n";

my @statfiles = @ARGV;
my %trim;
my @trimfiles = `ls trimreport*txt`;
chomp(@trimfiles);
foreach my $tfile (@trimfiles) {
  open FLAG, "<$tfile" or die $!;
  while (my $line = <FLAG>) {
    chomp($line);
    my ($sample,$file,$raw,$trim) = split(/\t/,$line);
    $trim{$sample}{rawct} += $raw;
    $trim{$sample}{trimct} += $trim;
  }
}
...
foreach $sfile (@statfiles) {
  ...
  close DUP;
  open DUP, "<$prefix\.subset.libcomplex.txt" or die $!;
  while (my $line = <DUP>) {
    chomp($line);
    if ($line =~ m/## METRICS/) {
      $header = <DUP>;
	...
	$info{$stats[$i]} = $row[$i];
      ...
      $hash{subset_percdups} = sprintf("%.4f",$info{PERCENT_DUPLICATION});
      $hash{libsize} = $info{ESTIMATED_LIBRARY_SIZE};
    }
  }
  close DUP;
  ...
  print OUT join("\t",$prefix,$hash{rawct}, $hash{total},$hash{pairs},$hash{maprate},$hash{propair},
		 ...
		 sprintf("%.2f",100*$hash{subset_percdups}),$hash{medinsert},$hash{avginsert},
		 $hash{stdinsert},$avgdepth,$median,$hash{'perc10x'},$hash{'perc50x'},$hash{'perc100x'},
		 $hash{'perc200x'},$hash{'perc500x'},$dedup_avgdepth,$dedup_median,$hash{'dedup.perc10x'},
		 $hash{'dedup.perc50x'},$hash{'dedup.perc100x'},$hash{'dedup.perc200x'},$hash{'dedup.perc500x'}),"\n";
}
