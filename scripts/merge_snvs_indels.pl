...
if ($somatic ne 'no_normal') {
 ...
 W1:while (my $line = <IN>) {
  ...
  F1:foreach my $subjid (@gtheader) {
    ...
    next if ($dp[0] < 20);
    next if ($maf[0][0] < 0.05 && ($maf[1][0] > 0.01 || $maf[1][0]*5 > $maf[0][0]));
    $hash{AF} = join(",",@{$maf[0]});
    ...
    delete $hash{SOMATIC};
    if ($maf[1][0] > 0.05 || $maf[1][0]*5 > $maf[0][0]) {
      $hash{Somatic} = 'Germline';
    }else {
      $hash{Somatic} = 'Somatic';
...
W1:while (my $line = <IN>) {
  ...
  if ($normals{$chrom}{$pos}) {
    ...
    my $refct = 0;
    $refct = $normalcts{$ref} if ($normalcts{$ref});
  ...
  foreach  my $i (0..$#acts) {
    if ($acts[$i] > 2) {
      if ($hash{NormalDP}) {
	my $normbasect = 0;
	$normbasect = $normalcts{$altnts[$i]} if ($normalcts{$altnts[$i]}) ;
	push @normaltct,$normbasect;
	push @normmaf,  sprintf("%.4f",$normbasect/$hash{NormalDP});
      }
  ...
  if ($normmaf[0] && ($normmaf[0] > 0.05 || $normmaf[0]*5 > $mutallfreq[0])) {
    next if ($mutallfreq[0] < 0.05);
    $hash{Somatic} = 'Germline';
  }
  if ($hash{DP} < 20) {
    $fail{'LowDepth'} = 1;
