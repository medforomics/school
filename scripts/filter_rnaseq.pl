...
open OM, "</project/shared/bicf_workflow_ref/GRCh38/panel1410.genelist.txt" or die $!;
while (my $line = <OM>) {
  chomp($line);
  $keep{$line} = 1;
}
...
while (my $line = <IN>) {
  ...
  if ($hash{dbNSFP_ExAC_Adj_AF}) {
      $exacaf = $hash{dbNSFP_ExAC_Adj_AF};
  }
  ...
  if (scalar(@fail) < 1) {
    $filter = 'PASS';
    print OUT join("\t","chr".$chrom,$pos,$id,$ref,join(",",@newalts),$score,
		   $filter,$annot,$format,$allele_info),"\n";
  }else {
    next;
  }
}
system("bgzip $prefix\.tumor.vcf");
system("tabix $prefix\.tumor.vcf.gz");
