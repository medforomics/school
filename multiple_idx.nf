...
new File(params.design).withReader { reader ->
    ...
    while (line = reader.readLine()) {
	   ...
	   if (fileMap.get(row[oneidx]) != null) {
	      prefix << tuple(row[prefixidx],row[mrgidx],fileMap.get(row[oneidx]),fileMap.get(row[twoidx]))
	   }
...
process trimpe {
  ...
  input:
  set pair_id, mergename, file(read1), file(read2) from read_pe
  output:
  set pair_id, mergename, file("${read1.baseName.split("\\.fastq")[0]}_val_1.fq.gz"), file("${read2.baseName.split("\\.fastq")[0]}_val_2.fq.gz") into trimpe
  file("${mergename}.${pair_id}.trimreport.txt") into trimstat
  script:
  """
  module load trimgalore/0.4.1 cutadapt/1.9.1
  trim_galore --paired -q 25 --illumina --gzip --length 35 ${read1} ${read2}
  perl $baseDir/scripts/parse_trimreport.pl ${mergename}.${pair_id}.trimreport.txt ${read1}_trimming_report.txt ${read2}_trimming_report.txt
  """
}
...
process alignpe {
  ...
  input:
  set pair_id, mergename, file(fq1), file(fq2) from trimpe
  output:
  set mergename,file("${pair_id}.bam") into aligned
  script:
  """
...
process mergebam {
  ...
  sambamba sort -t 20 -n -o output.namesort.bam merge.bam
  sambamba sort -t 20 -o output.sort.bam merge.bam
  sambamba markdup -t 20 output.sort.bam ${sid}.bam 
...
process seqqc {
  ...
  java -Xmx20g -jar \$PICARD/picard.jar EstimateLibraryComplexity INPUT=${pair_id}.ontarget.bam OUTPUT=${pair_id}.libcomplex.txt
