...
new File(params.design).withReader { reader ->
    ...
    twoidx = header.findIndexOf{it == 'ConsensusBAM'};
	...
	oribam << tuple(row[fidx],row[tidx],fileMap.get(row[oneidx]),fileMap.get(row[twoidx]))
	   }
...
process indexoribams {
  ...
  input:
  set sid,tid,file(tumor),file(consen) from oribam
  output:
  set sid,tid,file(tumor),file("${tumor}.bai") into aligned2
  set sid,tid,file(consen) into qcbam
