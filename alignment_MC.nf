...
process alignpe {
  ...
  samtools sort --threads \$SLURM_CPUS_ON_NODE -o output.sort.bam output.unsort.bam
  java -Xmx20g -jar \$PICARD/picard.jar MarkDuplicates I=output.sort.bam O=${pair_id}.bam M=${pair_id}.libcomplex.txt ASSUME_SORTED=true OPTICAL_DUPLICATE_PIXEL_DISTANCE=${params.fcdist}
  """
 }
...
process mergebam {
  ...
  if [ \$count -gt 1 ]
  then
  samtools merge --threads \$SLURM_CPUS_ON_NODE merge.bam *.bam
  else
  mv *.bam merge.bam
  fi
